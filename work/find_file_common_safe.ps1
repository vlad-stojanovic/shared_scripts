...
function find_absolute_path_in_start_dir() {
	...
	[string]$rootDirectoryDescription = ""
	...
	If ([System.IO.Path]::IsPathRooted($filePath)) {
		...
		If ($fullFilePaths.Count -Gt 0) {
			$rootDirectoryDescription = " in [$($startDir)]"
		} Else {
			$rootDirectoryDescription = " in current folder"
	...
	If ($fullFilePaths.Count -Gt 1) {
		LogErrorInFindingEntries -entryDescription "files$($rootDirectoryDescription)" -entries $fullFilePaths
		return $Null	
		...
		If ($filePathResults.Count -Eq 0) {
			Log Error -message "File [$($filePath)] not found in [$($startDir)]" -indentLevel 1
			return $Null
		} ElseIf ($filePathResults.Count -Gt 1) {
			LogErrorInFindingEntries -entryDescription "files in [$($startDir)]" -entries $filePathResults
			return $Null
		}
...
function find_project_for_file_in_start_dir() {
	...
	While ([string]::IsNullOrWhiteSpace($projectPath)) {
		...
		If ($projectFiles.Count -Gt 1) {
			LogErrorInFindingEntries -entryDescription "matching project files" -entries $projectFiles
			return $Null
